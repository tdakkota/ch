...
func (c *ColIPv4) DecodeColumn(r *Reader, rows int) error {
	...
	if len(data) > unrollByteSize {
		...
		for i = 0; i <= len(data)-unrollByteSize; i += unrollByteSize {
			...
			src[3] =
				IPv4(binary.LittleEndian.Uint32(data[i+offset0 : i+offset0+size : i+offset0+size]))
			const offset1 = 0 + size + size
			src[2] =
				IPv4(binary.LittleEndian.Uint32(data[i+offset1 : i+offset1+size : i+offset1+size]))
			const offset2 = 0 + size
			src[1] =
				IPv4(binary.LittleEndian.Uint32(data[i+offset2 : i+offset2+size : i+offset2+size]))
			const offset3 = 0
			src[0] =
				IPv4(binary.LittleEndian.Uint32(data[i+offset3 : i+offset3+size : i+offset3+size]))
	...
	for _ = i; i < len(data); i += size {
		v[n] =
			IPv4(binary.LittleEndian.Uint32(data[i:]))
		n++
	}
	*c = v
	return nil
