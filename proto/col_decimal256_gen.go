...
func (c *ColDecimal256) DecodeColumn(r *Reader, rows int) error {
	...
	v := append(*c, make([]Decimal256, rows)...)
	...
	if len(data) > unrollByteSize {
		...
		for i = 0; i <= len(data)-unrollByteSize; i += unrollByteSize {
			src := [unroll]Decimal256{}
			const offset0 = 0 + size + size + size
			src[3] =
				Decimal256(binUInt256(data[i+offset0 : i+offset0+size : i+offset0+size]))
			const offset1 = 0 + size + size
			src[2] =
				Decimal256(binUInt256(data[i+offset1 : i+offset1+size : i+offset1+size]))
			const offset2 = 0 + size
			src[1] =
				Decimal256(binUInt256(data[i+offset2 : i+offset2+size : i+offset2+size]))
			const offset3 = 0
			src[0] =
				Decimal256(binUInt256(data[i+offset3 : i+offset3+size : i+offset3+size]))
